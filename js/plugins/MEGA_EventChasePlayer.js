//=============================================================================
// MEGA_EventChasePlayer.js
//=============================================================================

var Imported = Imported || {};
Imported.MEGA_EventChasePlayer = true;
...
* @author Yanfly Engine Plugins, improve performance mega
*
* @param Sight Lock
* @type number
* @min 0
...
Yanfly.Parameters = PluginManager.parameters('MEGA_EventChasePlayer');
...
Game_Event.prototype.canSeePlayer = function () {
    ...
    ////// chan mod
    if ((this.direction() === 2 && direction !== 8) || (this.direction() === 6 && direction !== 4) || (this.direction() === 4 && direction !== 6) || (this.direction() === 8 && direction !== 2)) {
...
Game_Event.prototype.updateChaseMovement = function () {
    ...
    if (this._stopCount > 0 && this._chasePlayer) {
        //// chan mod 
        if(Math.randomInt(3) % 2 === 0) {
            var direction = this.findDirectionTo($gamePlayer.x, $gamePlayer.y);
            if (direction > 0) {
                var x = this._x;
                var y = this._y;
                this.moveStraight(direction);
                if (x === this._x && y === this._y) this._staggerCount = 10;
            }
        }
        else {
            var x = this._x;
